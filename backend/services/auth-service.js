...
export default class authService {
  ...
  async Register(body) {
    ...
    try {
      ...
      await sendEmail(email, "Welcome to BackendHub", "welcomeMail", {
        Name: `${user.firstName} ${user.lastName}`,
        Link: verificationUrl,
      });
      await sendEmail(email, "Verify Email", "verificationLinkMail", {
        Name: `${user.firstName} ${user.lastName}`,
        Link: verificationUrl,
      });
  ...
  async Login(body) {
    ...
    try {
      ...
      throw new APIError(
        "An error occurred while login a user",
        STATUS_CODES.INTERNAL_ERROR,
        err.message,
        false,
        err.stack
  ...
  async OAuthSignIn(body) {
    ...
    try {
      ...
      if (!existingUser.isVerified) {
        return {
          message:
            "User exist but user account is not verified. Please check your email.",
          status: 401,
        };
      }
  ...
  async forgotPasswordRequestUrl(body) {
    ...
    try {
      ...
      if (existingUser && existingUser.isVerified) {
        ...
        await sendEmail(email, "Forgot Password Link", "verificationLinkMail", {
        ...
        return {
          message:
            "you will be able to forgot your password. We sent you a mail.",
          status: 200,
        };
      }
      return {
      ...
      throw new APIError(
        "Error occurred while creating request url for password change",
        STATUS_CODES.INTERNAL_ERROR,
        err.message,
        false,
        err.stack
