...
const auth = (app) => {
  ...
  app.post("/api/v1/auth/request-forgot-password", async (req, res, next) => {
    try {
      const { status, ...rest } = await service.forgotPasswordRequestUrl(
        req.body
      );
      return res.status(status).json({ ...rest });
    } catch (err) {
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const { status, ...rest } = await service.ChangeUserPassword(
          req,
          token,
          newPassword
        );
  ...
  app.post("/api/v1/auth/changePassword", protect, async (req, res, next) => {
    try {
